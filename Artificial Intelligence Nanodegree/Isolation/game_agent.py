...
def custom_score(game, player):
    ...
    """
    # consider the ratio of number of own moves vs opponent moves, and bias towards higher number of own moves
    ...
    return (own_moves/(opp_moves+0.0001))# - own_moves/8.0)
...
def custom_score_2(game, player):
    ...
    float
        The heuristic value of the current game state to the specified player.
    """
    #humble about remaining moves - assume we'll be running out of room down this path, but opponent won't
    opponent = game.get_opponent(player)
    own_moves = len(game.get_legal_moves(player))
    opp_moves = len(game.get_legal_moves(opponent))
...
def custom_score_3(game, player):
    ...
    float
        The heuristic value of the current game state to the specified player.
    """
    # If an area is crowded, but we have moves, go for it to possibly lure in the opponent
    ...
    c = c - 2
    BLANK = 0
    full_5x5 = len( [(i, j) for j in range(5) for i in range(5)
                if i+c > 0 and j+r > 0 and i+c < game.height and j+r < game.width and game._board_state[i+c + (j+r) * game.height] != BLANK])
    
    return float(full_5x5)
