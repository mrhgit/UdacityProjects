...
TIME_LIMIT = 20 #150  # number of milliseconds before timeout
...
def play_round(cpu_agent, test_agents, win_counts, num_matches):#, move1, move2):
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        #fresh_game = Board(agent.player, test_agents[0].player)
        #move1 = random.choice(fresh_game.get_legal_moves())
        #move2 = random.choice(fresh_game.get_legal_moves())
        counts = play_round(agent, test_agents, wins, num_matches)#, move1, move2)
