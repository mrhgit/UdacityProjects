...
class WaypointUpdater(object):
    def __init__(self):
        ...
        self.beginning_of_final_stop = -1
    ...
    def insert_stop(self,wp_idx):
        if not self.stops_cleared:
            return
        self.stops_cleared = False
        wp_idx -= 6 # Let's stop before we really have to
    ...
    def restore_speeds(self):            
        for i, wp in enumerate(self.base_waypoints.waypoints):
            wp.twist.twist.linear.x = self.original_waypoints.waypoints[i].twist.twist.linear.x
    ...
    def clear_stops(self):
        if self.speed_limit and not self.stops_cleared:
            ...
            self.restore_speeds()
            #self.base_waypoints = copy.deepcopy(self.original_waypoints)
...
#        return closest
    ...
    def waypoints_cb(self, lane):
        ...
        self.original_waypoints = copy.deepcopy(lane)
