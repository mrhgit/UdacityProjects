...
//#include <iostream> // used for debug (cout)
//using namespace std;
...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // Make the assumption of zero roll, pitch by rotating out these terms (i.e. convert from world frame to body frame)
  // It will be necessary to convert our predicted pitch and roll, as well as the gyro measurements
  // Add the gyro measurements to the predicted pitch and roll
  // Put the roll and pitch back in by rotating them back in (i.e. convert body frame to world frame)
  ...
  Mat3x3F R;
  R(0,0) = 1;
  R(0,1) = sin_phi*tan_theta;
  R(0,2) = cos_phi*tan_theta;
  R(1,0) = 0;
  R(1,1) = cos_phi;
  R(1,2) = -sin_phi;
  R(2,0) = 0;
  ...
  float phi_dot   = R(0,0)*gyro.x + R(0,1)*gyro.y + R(0,2)*gyro.z;
  float theta_dot = R(1,0)*gyro.x + R(1,1)*gyro.y + R(1,2)*gyro.z;
  float psi_dot   = R(2,0)*gyro.x + R(2,1)*gyro.y + R(2,2)*gyro.z;
  ...
  if (0){
      ...
      predictedPitch = pitchEst + dtIMU * theta_dot;
      predictedRoll = rollEst + dtIMU * phi_dot;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  // x,y,z position advances by x,y,z velocities
  // x,y,z velocity advances by x,y,z accelerations
  // roll/pitch/yaw advances by body rates converted to world frame
  ...
  Mat3x3F Rbg; // rotate from body to world frame
  Rbg(0,0) = c_th*c_ps;
  Rbg(0,1) = s_ph*s_th*c_ps - c_ph*s_ps;
  Rbg(0,2) = c_ph*s_th*c_ps + s_ph*s_ps;
  Rbg(1,0) = c_th*s_ps;
  Rbg(1,1) = s_ph*s_th*s_ps + c_ph*c_ps;
  Rbg(1,2) = c_ph*s_th*s_ps - s_ph*c_ps;
  Rbg(2,0) = -s_th;
  Rbg(2,1) = c_th*s_ph;
  Rbg(2,2) = c_th*c_ph;

  float acc_x = Rbg(0,0)*accel[0] + Rbg(0,1)*accel[1] + Rbg(0,2)*accel[2]; // get world x component of body frame acceleration vec
  float acc_y = Rbg(1,0)*accel[0] + Rbg(1,1)*accel[1] + Rbg(1,2)*accel[2]; // get world y component of body frame acceleration vec
  float acc_z = Rbg(2,0)*accel[0] + Rbg(2,1)*accel[1] + Rbg(2,2)*accel[2]; // get world z component of body frame acceleration vec
  acc_z -= 9.81f; // compensate for gravity

  predictedState[0] += curState[3]*dt; // advance x position
  predictedState[1] += curState[4]*dt; // advance y position
  predictedState[2] += curState[5]*dt; // advance z position
  predictedState[3] += acc_x*dt; // advance x velocity
  predictedState[4] += acc_y*dt; // advance y velocity
  predictedState[5] += acc_z*dt; // advance z velocity
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0,0) = -c_th*s_ps;
  RbgPrime(0,1) = -s_ph*s_th*s_ps - c_ph*c_ps;
  RbgPrime(0,2) = -c_ph*s_th*s_ps + s_ph*c_ps;
  RbgPrime(1,0) = c_th*c_ps;
  RbgPrime(1,1) = s_ph*s_th*c_ps - c_ph*s_ps;
  RbgPrime(1,2) = c_ph*s_th*c_ps + s_ph*s_ps;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  hPrime.block(0,0,6,6).setIdentity(); // direct relationship - identity measurement model (for these state vars)
  zFromX = ekfState.head(6); // direct relationship between state and measurements - no conversion necessary
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // Yaw is equivalent in meas and state spaces and there's no correlation to other measurements,
  // so using a bunch of matrix and vector math seems excessive, but thus be it written by Udacity.

  hPrime(0,6) = 1;
  zFromX(0) = ekfState(6); // the "measurement" from the State (X) is the yaw - no conversion necessary

  // let's make sure a z - zFromX gives us the minimum phase difference
